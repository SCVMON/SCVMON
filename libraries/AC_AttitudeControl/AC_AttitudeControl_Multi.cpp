...
{
    float roll_p=0, roll_i=0, roll_d=0, roll_ff=0;
    float pitch_p=0, pitch_i=0, pitch_d=0, pitch_ff=0;
    float yaw_p=0, yaw_i=0, yaw_d=0, yaw_ff =0;
...
/*
    if (t_a1 > 30000){
        ...
        gyro_latest.y = 10;
        gyro_latest.z = 10;
        get_rate_roll_pid().kP(0.35);
        get_rate_roll_pid().kD(0.6);
        get_rate_roll_pid().kI(0.03);
        get_rate_pitch_pid().kP(0.35);
        get_rate_pitch_pid().kI(0.6);
        get_rate_pitch_pid().kD(0.03);
        get_rate_yaw_pid().kP(0.6);
        get_rate_yaw_pid().kI(0.06);
        get_rate_yaw_pid().kD(0.02); 
    }
*/

    gyro_latest.x = _scv_gyro_x.monitor_SCV(gyro_latest.x, _scv_gyro_x.gyro_x);
    gyro_latest.y = _scv_gyro_y.monitor_SCV(gyro_latest.y, _scv_gyro_y.gyro_y);
    gyro_latest.z = _scv_gyro_z.monitor_SCV(gyro_latest.z, _scv_gyro_z.gyro_z);

    roll_p = get_rate_roll_pid().kP();
    roll_i = get_rate_roll_pid().kI();
    roll_d = get_rate_roll_pid().kD();
    roll_ff = get_rate_roll_pid().ff();
        
    pitch_p = get_rate_pitch_pid().kP();
    pitch_i = get_rate_pitch_pid().kI();
    pitch_d = get_rate_pitch_pid().kD();
    pitch_ff = get_rate_pitch_pid().ff();

    yaw_p = get_rate_yaw_pid().kP();
    yaw_i = get_rate_yaw_pid().kI();
    yaw_d = get_rate_yaw_pid().kD();
    yaw_ff = get_rate_yaw_pid().ff();

    roll_p = _scv_roll_p.monitor_SCV(roll_p, _scv_roll_p.roll_p);
    roll_i = _scv_roll_i.monitor_SCV(roll_i, _scv_roll_i.roll_i);
    roll_d = _scv_roll_d.monitor_SCV(roll_d, _scv_roll_d.roll_d);
    roll_ff =_scv_roll_ff.monitor_SCV(roll_ff, _scv_roll_ff.roll_ff);

    pitch_p = _scv_pitch_p.monitor_SCV(pitch_p, _scv_pitch_p.pitch_p);
    pitch_i = _scv_pitch_i.monitor_SCV(pitch_i, _scv_pitch_i.pitch_i);
    pitch_d = _scv_pitch_d.monitor_SCV(pitch_d, _scv_pitch_d.pitch_d);
    pitch_ff = _scv_pitch_ff.monitor_SCV(pitch_ff, _scv_pitch_ff.pitch_ff);

    yaw_p = _scv_yaw_p.monitor_SCV(yaw_p, _scv_yaw_p.yaw_p);
    yaw_i = _scv_yaw_i.monitor_SCV(yaw_i, _scv_yaw_i.yaw_i);
    yaw_d = _scv_yaw_d.monitor_SCV(yaw_d, _scv_yaw_d.yaw_d);
    yaw_ff = _scv_yaw_ff.monitor_SCV(yaw_ff, _scv_yaw_ff.yaw_ff);

    get_rate_roll_pid().kP(roll_p);
    get_rate_roll_pid().kI(roll_i);
    get_rate_roll_pid().kD(roll_d);
    get_rate_roll_pid().ff(roll_ff);

    get_rate_pitch_pid().kP(pitch_p);
    get_rate_pitch_pid().kI(pitch_i);
    get_rate_pitch_pid().kD(pitch_d);
    get_rate_pitch_pid().ff(pitch_ff);

    get_rate_yaw_pid().kP(yaw_p);
    get_rate_yaw_pid().kI(yaw_i);
    get_rate_yaw_pid().kD(yaw_d);
    get_rate_yaw_pid().ff(yaw_ff);


    //var[_scv_roll_p.roll_p] = roll_p;
    //var[_scv_roll_p.roll_i] = roll_i;
    //var[_scv_roll_p.roll_d] = roll_d;
    //var[_scv_roll_p.roll_ff] = roll_ff;

    //var[_scv_roll_p.pitch_p] = pitch_p;
    //var[_scv_roll_p.pitch_i] = pitch_i;
    //var[_scv_roll_p.pitch_d] = pitch_d;
    //var[_scv_roll_p.pitch_ff] = pitch_ff;

    //var[_scv_roll_p.yaw_p] = yaw_p;
    //var[_scv_roll_p.yaw_i] = yaw_i;
    //var[_scv_roll_p.yaw_d] = yaw_d;
    //var[_scv_roll_p.yaw_ff] = yaw_ff;

    //var[_scv_roll_p.gyro_x] = gyro_latest.x;
    //var[_scv_roll_p.gyro_y] = gyro_latest.y;
    //var[_scv_roll_p.gyro_z] = gyro_latest.z;
